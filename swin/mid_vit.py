...
class HybridFusionResNet(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.rgb_branch = timm.create_model('swin_large_patch4_window7_224.ms_in22k', pretrained=True, features_only=True)
        self.thermal_branch = timm.create_model('swin_large_patch4_window7_224.ms_in22k', pretrained=True, features_only=True)
        self.event_branch = timm.create_model('swin_large_patch4_window7_224.ms_in22k', pretrained=True, features_only=True)
        ...
        # Fusion layer: 1x1 convolution after concatenation
    ...
    def forward(self, rgb, thermal, event):
        # Extract features up to the second residual block (index 2)
        rgb_features = self.rgb_branch(rgb)[2]
        thermal_features = self.thermal_branch(thermal)[2]
        event_features = self.event_branch(event)[2]
...
def main():
    ...
    # Initialize the model with fusion at the second block
    model = HybridFusionResNet(num_classes=2).to(device)
